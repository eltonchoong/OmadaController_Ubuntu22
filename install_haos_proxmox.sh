...
qm create $VMID --name $VM_NAME --memory $MEMORY --sockets $CPU_SOCKETS --cores $CPU_CORES --cpu host --bios ovmf --efidisk0 local-lvm:vm-$VMID-disk-0,efitype=4m,format=raw --ostype l26 --scsihw virtio-scsi-pci --net0 virtio,bridge=vmbr0

# Download HAOS image
echo "🌐 Downloading Home Assistant OS image (overwrite if exists)..."
cd /tmp
